...
def draw_epoch(n):
    ...
    if casper.get_main_hash_finalized():
        ...
        G.add_node(n-1, color='blue', style='filled')
    else:
        G.add_node(n)

    msg="current epoch: {}\n \
        total currdyn deposits: {}\n \
        total prevdyn deposits: {}\n \
        Penalty factor: {: 0.8f}\n".format(
        casper.get_current_epoch(), \
        casper.get_total_curdyn_deposits(), \
        casper.get_total_prevdyn_deposits(), \
        casper.get_current_penalty_factor()
    )
    if n != 1:
...
assert casper.get_main_hash_justified()
s.mine(1)
print("Epoch 12 prepare message processed")
...
draw_epoch(14)
s.mine(EPOCH_LENGTH * 15 - s.head_state.block_number)
casper.initialize_epoch(15)

draw_epoch(15)
s.mine(EPOCH_LENGTH * 16 - s.head_state.block_number)
casper.initialize_epoch(16)

draw_epoch(16)
s.mine(EPOCH_LENGTH * 17 - s.head_state.block_number)
casper.initialize_epoch(17)

draw_epoch(17)
s.mine(EPOCH_LENGTH * 18 - s.head_state.block_number)
casper.initialize_epoch(18)

saveToDotGraph(G, 'chain')
